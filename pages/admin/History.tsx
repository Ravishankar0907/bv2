...
const History: React.FC = () => {
  const { orders, updateOrderStatus, addOrderExpense, removeOrderExpense } = useStore();
                ...
                return (
                    <div key={order.id} className={`bg-dark-card p-4 rounded-xl border flex flex-col md:flex-row gap-4 items-center ${isReturned ? 'border-brand-500/50' : 'border-slate-800'}`}>
                        <div className="flex items-center gap-4 flex-1 w-full">
                            <img src={order.productImage} alt="" className="w-12 h-12 rounded bg-slate-800 object-cover" />
                            <div>
